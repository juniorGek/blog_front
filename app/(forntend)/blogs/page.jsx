...
const Page = () => {
    ...
    useEffect(() => {
        if (param.get('category_id')) {
            getCategory({ _id: param.get('category_id') });
            getBlogs({ category_id: param.get('category_id'), search: "", subcategory_id: '' });
        } else if (param.get('tag_id')) {
            getBlogs({ tag_id: param.get('tag_id') });
        } else if (param.get('search')) {
            getBlogs({ search: param.get('search') });
        } else if (param.get('subcategory_id')) {
            getSubCategory({ _id: param.get('subcategory_id') });
            getBlogs({ subcategory_id: param.get('subcategory_id'), search: "", category_id: '' });
        } else {
    ...
    useEffect(() => {
        getEdChoice({ category_id: param.get('category_id') });
    }, [param]);
    ...
    return (
        <Suspense fallback={<div>Chargement...</div>}>  {/* Ajout de la frontière Suspense ici */}
                        ...
                        <div className="lg:flex justify-between gap-x-5">
                                ...
                                <div className='my-8'>
                                    <div className="flex items-center justify-center mt-8">
                                        <Pagination2
                                            ...
                                            onSizeChange={size => getBlogs({ size })}
                                            limit={blogs?.limit}
                            ...
                            <div className="basis-1/3 md:mt-0 mt-10">
                                <div className="border dark:border-BG_Line_Color border-Light_Line_Color p-5 relative rounded-lg">
                                    <Input
                                        onChange={(values) => push(`/blogs/?search=${values.target.value}`)}
                                        className='paragraph_2 text-Font2_Color p-5 w-full bg-Primary_Color bg-opacity-10 outline-none border-none dark:placeholder:text-white'
                                        placeholder={i18n?.t('Search Here')}
                                        type="text"
                                    />
                                    <button className='text-[28px] absolute top-1/2 right-8 hover:text-Primary_Color duration-300 text-Font2_Color -translate-y-1/2'>
                                        <BiSearch />
                                    </button>
                                </div>
                                <div className="rounded-lg border dark:border-BG_Line_Color border-Light_Line_Color p-6 my-10">
                                    <h1 className='header_4 pb-4 border-b dark:border-BG_Line_Color border-Light_Line_Color text-Font1_Color dark:text-white'>
                                        {i18n?.t('EDITOR CHOICE')}
                                    </h1>
                                    <div className="">
                                        {edChoice?.map((e) => {
                                            return (<ChoiceCard item={e} key={e.id} />)
                                        })}
                                        {!edChoice && (
                                            <div className=''>
